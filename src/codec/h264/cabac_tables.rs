//! CABAC lookup tables from ITU-T H.264 tables 9-12 to 9-24, 9-44 and 9-45.
//!
//! Generated data, not written by hand. The values were transcribed
//! mechanically from the openh264 reference decoder
//! (`codec/common/src/common_tables.cpp`, Cisco Systems, BSD-2-Clause), which
//! labels them with the same spec table numbers. They are the standard's
//! tables rather than any implementation's invention, but the provenance is
//! recorded here because the transcription came from that file.
//!
//! Do not edit by hand. The structural invariants in `super::cabac`'s tests
//! guard against corruption.

/// `rangeTabLPS`, spec table 9-44, indexed by `[pStateIdx][qCodIRangeIdx]`.
...
/// `transIdxLPS`, spec table 9-45: the next state after coding an LPS.
...
/// `transIdxMPS`, spec table 9-45: the next state after coding an MPS.
...
/// Context initialisation `(m, n)` pairs, spec tables 9-12 to 9-24.
///
/// Indexed by `[ctxIdx][variant]`, where variant 0 is I slices and variants
/// 1..=3 are P and B slices with `cabac_init_idc` 0, 1 and 2. Entries the
/// spec marks "na" are stored as `(0, 0)`; they belong to syntax elements
/// that cannot occur for that slice type, so a correct decoder never reads
/// them. [`super::cabac::ContextVariant`] selects the column.
///
/// 460 contexts covers `ctxIdx` 0..=459, which is every context needed for
/// frame-coded 4:2:0 including the 8x8 transform. The spec's 460..=1023 range
/// is 4:4:4 only, which this decoder rejects at the parameter-set stage.
pub const CONTEXT_INIT: [[(i8, i8); 4]; 460] = [
