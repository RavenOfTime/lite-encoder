...
## `openh264-256x128-cabac-2slice.h264`

Deterministic three-picture synthetic stream generated by OpenH264, with each
picture split into exactly two CABAC slices. It keeps picture assembly covered
by default `cargo test`, without compiling or linking the optional C decoder.
The test locks dimensions, timestamps, and per-plane pixel fingerprints; the
`reference-decoder` variant additionally proves bit-exact OpenH264 agreement.

Regenerate it with:

```console
cargo run --example gen_h264_multislice_fixture --features reference-decoder -- \
  tests/fixtures/openh264-256x128-cabac-2slice.h264
```
